...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | h :: t => if leb (count h s1) (count h s2) then (included t s2) else false
  end.
...
Proof.
  (* `n` denotes the head of the bag. `s'` denotes the tail of the bag*)
  (* and `Ihs'` denotes the inductive hypothesis for the tail of the bag (list)*)
